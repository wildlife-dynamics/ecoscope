...
class NewRelocations(StraighttrackMixin):
    """
...
class NewTrajectory(StraighttrackMixin):
    """
    ...
    def from_relocations(self, *args, **kwargs):
        ...
        self._gdf = (
            self._gdf.groupby("groupby_col")[self._gdf.columns]
            .apply(NewTrajectory._create_multitraj)
            .droplevel(level=0)
        )
        self._gdf.to_crs(crs, inplace=True)
    ...
    def _create_multitraj(df):
        ...
        return NewTrajectory._create_trajsegments(df[:-1])
