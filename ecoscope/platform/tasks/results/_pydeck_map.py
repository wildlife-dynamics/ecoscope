...
class PathLayerStyle(LayerStyleBase):
    ...
    rounded: Annotated[bool, AdvancedField(default=False)] = False
    extruded: Annotated[bool, AdvancedField(default=False)] = False
    wireframe: Annotated[bool, AdvancedField(default=True)] = True
...
def _custom_tile_layer_json_schema() -> dict:
    schema = TiledBitmapLayerDefinition.model_json_schema()
...
def _preset_or_custom_json_schema_extra(schema: dict) -> None:
    ...
    schema["default"] = [
        TiledBitmapLayerDefinition(layer_name="TERRAIN")._as_json_schema_default(),
        TiledBitmapLayerDefinition(layer_name="SATELLITE", opacity=0.5)._as_json_schema_default(),
    ]
